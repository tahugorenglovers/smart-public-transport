...
def detect_anomaly(data: AnomalyRequest):
    # Rule-based detection: speed > 100 km/h or stop_duration < 3s = anomaly
    # (ML model output is used as secondary signal)
    is_anomaly = data.speed > 100 or data.stop_duration < 3
    ...
    if is_anomaly:
        try:
            ...
            ch = conn.channel()
            ch.exchange_declare(exchange="smarttransit", exchange_type="topic", durable=True)
            ch.queue_declare(queue="env.anomaly.detected", durable=True)
            ch.queue_bind(exchange="smarttransit", queue="env.anomaly.detected",
                          routing_key="bus.anomaly.detected")
            ch.basic_publish(
                ...
                body=json.dumps({
                    "bus_id": data.bus_id,
                    "severity": severity,
                    "message": f"Bus {data.bus_id} anomaly detected — speed {data.speed} km/h"
                }),
                properties=pika.BasicProperties(delivery_mode=2)
            )
            conn.close()
            print(f"[Anomaly] Published bus.anomaly.detected bus_id={data.bus_id}")
        except Exception as e:
            print(f"[Anomaly] RabbitMQ publish failed: {e}")
